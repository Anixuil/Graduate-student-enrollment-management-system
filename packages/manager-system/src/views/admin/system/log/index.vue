<template>
    <div class="log-wrapper">
        ...
        >
            ...
            >
                <el-tag v-if="item.prop === 'logTitle'" type="success">
                    {{ row[item.prop] }}
                </el-tag>
                <el-tag v-else-if="item.prop === 'index'">{{ index + 1 }}</el-tag>
                <el-tag
                    v-else-if="item.prop === 'logStatus'"
                    :type="
                        row[item.prop] == 200
                            ? 'success'
                            : row[item.prop] == 400
...
import { logTableOption } from './index'
import { Ref } from 'vue'
...
const data: any = ref([])
const option = reactive(logTableOption)
